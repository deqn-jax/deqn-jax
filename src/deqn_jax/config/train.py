...
class TrainConfig(_ConfigBase):
    ...
    n_quadrature_points: int = Field(
        default=3,
        description="Quadrature points per shock dimension when `expectation_type` is `quadrature`/`gh`/`gauss_hermite` (total nodes = n_quadrature_points^n_shocks). Ignored by `monomial`, whose node count is always 2*n_shocks.",
    )
