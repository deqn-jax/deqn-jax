...
def step(
    state: Array, policy: Array, shock: Array, constants: Dict, d_disaster: float = 0.0
) -> Array:
    """State transition.
    ...
    mu_ups_next = jnp.exp(
        c["rho_mu_ups"] * safe_log(st.mu_ups) + c["sigma_mu_ups"] * mu_ups_shock
    )
    mu_z_next = jnp.exp(
        c["rho_mu_z"] * safe_log(st.mu_z)
        + (1 - c["rho_mu_z"]) * safe_log(c["mu_z_ss"])
        + c["sigma_mu_z"] * mu_z_shock
    )
    g_next = jnp.exp(
        c["rho_g"] * safe_log(st.g)
        + (1 - c["rho_g"]) * safe_log(c["g_ss"])
        + c["sigma_g"] * g_shock
    )
...
_SIM_UPPER = jnp.array(
    [1.3, 80.0, 5.0, 3.0, 3.0, 1.15, 5.0, 8.0, 1.4, 1.3, 1.2, 1.04, 3.0]
)
...
_SOFT_UPPER = jnp.array(
    [3.5, 100.0, 8.0, 5.0, 5.0, 3.5, 6.0, 10.0, 3.5, 3.5, 3.0, 3.5, 5.0]
)
