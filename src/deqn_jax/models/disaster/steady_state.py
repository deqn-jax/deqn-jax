...
from deqn_jax.models.disaster.equations import equations
...
def _solve_steady_state(constants: Dict) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Unknowns: 12 policy variables. State constructed from them.
    Uses JAX autodiff Jacobian for robust convergence.
    """
    c = constants
    ...
    def _build_state(x):
        """Construct SS state from 12 policy variables."""
        lambda_z, i, pi, cc, w_tilda, s, omega_bar, h, F_w, F_p, q, L = x
        mu_z = c["mu_z_ss"]
