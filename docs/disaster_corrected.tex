...
\item \emph{July 2026.} Documentation audit (independently refereed
  counterfactual re-solves): the $\pi_{ss}$-gap mechanism paragraph
  rewritten (the pin is the Taylor rule's output-gap term against a
  miscalibrated $y_{\text{ss}}$ constant, \emph{not} financial-friction
  compounding); ELB SS-distortion claim corrected
  (${\sim}10^{-7}$ holds only locally on $R$; the equilibrium effect on
  solved $\pi_{ss}$ is $-8\times 10^{-5}$ in logs); $R_{\text{ss}}$
  constant vs solved-SS rate disambiguated; stale policy-bounds-table
  SS values and stale equation numbers in the provenance table fixed.
  See errata item~12.
\end{itemize}
...
$K_{p,t}$        & 1.00 & $\infty$ & 4.7774 & softplus \\
$K_{w,t}$        & 0.50 & $\infty$ & 2.1825 & softplus \\
\bottomrule
\end{tabular}
...
\item $C^{\infty}$ in $R^{\text{Taylor}}_t$ but with a sharp transition
  ...
  $R_t \approx R^{lb}$.
\item At the steady state the floor is slack: the \emph{solved} SS policy
  rate is $1.0180$ (distinct from the $R_{\text{ss}}$ \emph{constant}
  $1.011678$ in the calibration table --- see the SS note), well above
  $R^{lb}$. The softplus wedge on $R$ itself is
  ${\sim}2.5\times 10^{-7}$ at $\sharp = 500$; its \emph{equilibrium}
  effect on solved $\pi_{ss}$ is $-8\times 10^{-5}$ in logs (amplified
  ${\times}6.67$ by interest smoothing $1/(1-\rho_p)$ and ${\times}2$ by
  $1/(\alpha_\pi - 1)$; measured by counterfactual re-solve, July 2026).
...
$q_t$           & Investment Euler (Eq.~7) & network output \\
$i_t$           & Investment Euler (Eq.~7) & network output \\
$\tilde{w}_t$   & Wage Phillips (Eq.~3, 4a/4b) & network output \\
$c_t$           & Consumption Euler (Eq.~5, with Eq.~9 enforcing identity) & network output \\
...
SS interest rate & $R_{\text{ss}}$ & 1.011678 & Steady state (Fisher-consistent value: 1.0116421) \\
SS output & $y_{\text{ss}}$ & 3.0308 & \emph{Miscalibrated} (solved-SS GDP $= 3.0044$; see \S\ref{sec:steady_state}) \\
...
\item $R^{lb}_{\text{sharpness}} = 500$ keeps the softplus wedge on $R$
  at ${\sim}2.5\times 10^{-7}$ (equilibrium effect on solved
  $\pi_{ss}$: $-8\times 10^{-5}$ in logs; see \S\ref{sec:elb}) while
  still producing a tight kink near the floor.
...
\textbf{Note on the target/solved gap (mechanism corrected July 2026).}
The CMR calibration targets $\pi_{ss} = 1.006$, but the nonlinear solver
lands at $\pi = 1.0123$, a $+0.6$pp gap. \emph{The pin is the monetary
block}: at any stationary point the bond Euler fixes the real rate
(Fisher: $R = \pi\,\mu_z/\beta$), and the Taylor rule must produce that
same $R$ --- but its output-gap term $(y^{\text{GDP}}/y_{\text{ss}})^{\alpha_y}$
permanently drags the prescribed rate because the model's actual SS GDP
($g + c + i/\mu_\Upsilon = 3.0044$, from the state/policy tables on this
page) sits $0.87\%$ below the hardcoded $y_{\text{ss}} = 3.0308$
constant. With Taylor slope $\alpha_\pi = 1.5$ against Fisher slope 1,
the equilibrium shifts by
$\Delta\log\pi = -(\delta_R + \alpha_y\,\Delta\log y)/(\alpha_\pi - 1)$,
where $\delta_R$ is the log gap between the $R_{\text{ss}}$ constant and
the Fisher-consistent rate: predicted $+0.006233$ vs.\ actual
$+0.006230$. Counterfactual re-solves (July 2026, independently
refereed): $\alpha_y := 0 \Rightarrow \pi^* = 1.005844$;
$y_{\text{ss}} := 3.006808$ and $R_{\text{ss}} := 1.0116421$ (Fisher-consistent)
with the ELB wedge removed $\Rightarrow \pi^* = 1.006000$ exactly. An
indexation sweep $\iota = \iota_w \in \{0, 1\}$ moves $\pi^*$ by
$\leq 0.07$pp --- the Calvo/indexation block is \emph{not} the
mechanism, and an earlier draft of this note that attributed the gap to
financial-friction compounding is refuted by the same counterfactuals.
The SS at $1.0123$ is internally consistent (all 11 residuals
$< 6\times 10^{-8}$); it is the correct SS \emph{of the shipped
constants}, which encode a $5\%$-inflation economy rather than the
$2.4\%$ target. The exact recalibration exists but roughly doubles the
model's ELB exposure ($1.18\sigma/{\sim}12\%$ of quarters at shipped
values $\to 0.77\sigma/{\sim}22\%$) --- adopting it is a modeling
decision, not a bug fix, and must be taken jointly with the ELB
treatment.
...
\item \textbf{Documentation audit} (July 2026; equations audited against
  the code and re-derived, both by hand and by an independent
  cross-family referee --- \emph{no errors found in the equations
  themselves}; the SS tables pass independent consistency checks
  through Eq.~2a and Eq.~9). Prose corrections: (a) the
  $\pi_{ss}$-gap mechanism note rewritten --- the pin is the Taylor
  output-gap term against the miscalibrated $y_{\text{ss}}$ constant,
  established by counterfactual re-solves; the earlier
  financial-friction-compounding story was wrong; (b) ELB SS-distortion
  claim corrected from ${\sim}10^{-7}$ (which holds only for the local
  wedge on $R$) to $-8\times 10^{-5}$ log-$\pi_{ss}$ equilibrium effect;
  (c) solved-SS rate ($1.0180$) vs $R_{\text{ss}}$ constant
  ($1.011678$) disambiguated; (d) policy-bounds table $K_p/K_w$ SS
  values refreshed to the post-May-2026 solve ($4.7774$, $2.1825$);
  (e) provenance-table equation numbers updated to the current
  numbering (Eq.~5, Eq.~7).
\end{enumerate}
