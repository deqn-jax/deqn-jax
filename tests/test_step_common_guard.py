...
def test_train_step_variant_is_bit_identical_to_reference(case_name):
    ref = np.load(REFERENCE, allow_pickle=False)
    losses, param_hash = run_case(build_cases()[case_name])

    assert np.array_equal(losses, ref[f"{case_name}__loss"]), (
        f"{case_name}: loss history moved\n"
        f"  got      {losses}\n"
        f"  expected {ref[f'{case_name}__loss']}"
    )
