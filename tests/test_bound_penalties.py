...
def test_lower_violation_scales_with_square():
    ...
    assert float(_compute_bound_penalty(values, spec)) == pytest.approx(expected, rel=1e-6)
...
def test_both_bounds_stack():
    ...
    spec = {"c": {"lower": 0.0, "upper": 1.0,
                  "penalty_lower": 1.0, "penalty_upper": 1.0}}
...
def _loss_with_bound(bound_kind: str, **kwargs):
    ...
    net = mlp_mod.create_mlp(
        n_states=model.n_states, n_policies=model.n_policies,
        hidden_sizes=(8,), activation="tanh",
        policy_lower=model.policy_lower, policy_upper=model.policy_upper,
        key=key,
    )
    ...
    total, eq_losses = compute_loss(
        model=model, policy_fn=jax.vmap(net),
        states=states, key=jax.random.PRNGKey(1), mc_samples=2,
    )
