...
- Observe the **canonical carrier** named in the brief: the built artifact, the live endpoint, the migrated table, the final checkpoint on the DGX. Never the source that should have produced it; never a cached or scratch derivative.
...
- Do not touch the working copy — it is shared with the author: no git command that writes the working tree, the index or refs (`checkout`, `switch`, `restore`, `reset`, `stash`, `clean`, `apply` are examples, not the list); rebuild the carrier from the current tree, read another revision through `git show <ref>:<path>`.
